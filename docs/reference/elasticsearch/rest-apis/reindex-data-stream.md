...
::::{tip}
These APIs are designed for indirect use by {{kib}}'s **Upgrade Assistant**. We strongly recommend you use the **Upgrade Assistant** to upgrade from 8.17 to {{stack-version}}. For upgrade instructions, refer to [Upgrading to Elastic {{stack-version}}](docs-content://deploy-manage/upgrade/deployment-or-cluster.md).
::::
